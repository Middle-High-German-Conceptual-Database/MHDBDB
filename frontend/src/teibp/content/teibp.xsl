...
<xsl:stylesheet version="1.0"
  ...
  <!-- modify filePrefix to point to files on your own server, 
	 ...
	 <xsl:value-of select="normalize-space(key('ids',$rendition-id)/text())"/>
       ...
       <xsl:template match="tei:rendition[ @scheme = 'css' or not( @scheme ) ]" mode="rendition2style">
	 <!-- 
	      handle elements that point to this rendition with
	      @rendition (as supported by the Guidelines), and also
	      those that this <rendition> points to via its @corresp
	 -->	      
	 <xsl:variable name="points-to">
	   <xsl:choose>
	     <xsl:when test="@xml:id"><xsl:value-of select="@xml:id"/></xsl:when>
	     <xsl:when test="@corresp">
	       <!-- Note: this only handles a value with 1 pointer -->
	 ...
	 <xsl:choose>
	   <!-- 
		In TEI P5 version 2.9.1 and later <rendition> has a
		@selector attribute:
	   -->
	   <xsl:when test="@selector">
	     <xsl:value-of select="@selector"/>
	   </xsl:when>
	   <xsl:when test="string-length( $points-to ) > 0">
	     <xsl:value-of select="concat('*[rendition~=&quot;#', $points-to,'&quot;]')"/>
	   </xsl:when>
	 </xsl:choose>
	 <xsl:if test="@scope">

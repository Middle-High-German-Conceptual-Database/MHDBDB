...
<xsl:stylesheet version="1.0"
	...
	<xsl:template name="tagUsage2style">
		<style type="text/css" id="tagusage-css">
			<xsl:for-each
				select="//tei:namespace[@name ='http://www.tei-c.org/ns/1.0']/tei:tagUsage">
    ...
    <xsl:value-of
			select="normalize-space(key('ids',$rendition-id)/text())" />
	</xsl:template>
	...
	<xsl:variable name="htmlFooter">
		<!-- <footer> Powered by <a href="{$teibpHome}">TEI Boilerplate</a>. TEI Boilerplate is
		licensed under a <a
	    ...
	    <xsl:variable
			...
			<xsl:choose>
				<xsl:when test="starts-with($facs,'#')">
	                ...
	                <xsl:variable
						name="vMimeType" select="'image/jpeg'" />
	                <xsl:value-of

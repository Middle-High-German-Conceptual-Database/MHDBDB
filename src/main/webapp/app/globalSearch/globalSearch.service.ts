...
export class GlobalSearchService extends MhdbdbIdLabelEntityService<
  GlobalSearchQueryParameterI,
  GlobalSearchFilterI,
  GlobalSearchOptionsI,
  GlobalSearchEntityClass
> {
  ...
  protected _sparqlQuery(qp: GlobalSearchQueryParameterI, countResults: boolean = false): string {
    let selectors = [];
    ...
    if (
      (qp.filter.isClassFilterActive && qp.filter.classFilter.includes(('Person' as unknown) as classFilterT)) ||
      !qp.filter.isClassFilterActive
    ) {
      selectors.push(selectorPerson);
    }
    if (
      (qp.filter.isClassFilterActive && qp.filter.classFilter.includes(('Place' as unknown) as classFilterT)) ||
      !qp.filter.isClassFilterActive
    ) {
      selectors.push(selectorPlace);
    }
    if (
      (qp.filter.isClassFilterActive && qp.filter.classFilter.includes(('Word' as unknown) as classFilterT)) ||
      !qp.filter.isClassFilterActive
    ) {
      selectors.push(selectorWord);
    }
    if (
      (qp.filter.isClassFilterActive && qp.filter.classFilter.includes(('Work' as unknown) as classFilterT)) ||
      !qp.filter.isClassFilterActive
    ) {
      selectors.push(selectorWork);
    }
    if (
      (qp.filter.isClassFilterActive && qp.filter.classFilter.includes(('conceptualSystem' as unknown) as classFilterT)) ||
      !qp.filter.isClassFilterActive
    ) {
      selectors.push(selectorConceptualSystem);
    }
    if (
      (qp.filter.isClassFilterActive && qp.filter.classFilter.includes(('nameSystem' as unknown) as classFilterT)) ||
      !qp.filter.isClassFilterActive
    ) {

...
public class WorkController extends ControllerBase {
    ...
    throws JSONException, IOException {
        ...
        String query = new StringBuilder()
            .append(this.getSparqlPrefixes())
            .append(System.lineSeparator() + "select distinct ?id ?label ?sameAs ?dateOfCreation ?authorId ?authorSameAs ?authorLabel ?authorRole ?instance ?instanceLabel ?expression ?expressionLabel ?genreForm ?genreFormMainParent ?bibTitle ?bibLicense ?bibPlace ?bibAgent ?bibDate where {")
            ...
            .append(System.lineSeparator() + "  OPTIONAL { ?bibId dct:license ?bibLicense . }")
            .append(System.lineSeparator() + "  OPTIONAL {")
            .append(System.lineSeparator() + "    ?bibId bf:provisionActivity ?bibProvisionActivity .")
            .append(System.lineSeparator() + "    OPTIONAL { ?bibProvisionActivity bf:place/rdfs:label ?bibPlace . }")
            .append(System.lineSeparator() + "    OPTIONAL { ?bibProvisionActivity bf:agent/rdfs:label ?bibAgent . }")
